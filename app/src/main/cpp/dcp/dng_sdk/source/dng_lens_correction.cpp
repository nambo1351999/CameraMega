...
bool dng_warp_params::IsRadNOP (uint32 ) const
	{
...
bool dng_warp_params::IsTanNOP (uint32 ) const
	{
...
dng_point_real64 dng_warp_params_rectilinear::EvaluateTangential (uint32 plane,
	...
	return dng_point_real64 (kt0 * (r2 + 2.0 * dvdv) + (2.0 * kt1 * dh * dv),  
							 kt1 * (r2 + 2.0 * dhdh) + (2.0 * kt0 * dh * dv)); 
...
bool dng_warp_params_fisheye::IsRadNOP (uint32 ) const
	{
...
bool dng_warp_params_fisheye::IsTanNOP (uint32 ) const
	{
...
dng_point_real64 dng_warp_params_fisheye::EvaluateTangential (uint32 ,
															  real64 ,
															  const dng_point_real64 & ,
															  const dng_point_real64 & ) const
	{
...
dng_point_real64 dng_warp_params_fisheye::MaxSrcTanGap (dng_point_real64 ,
														dng_point_real64 ) const
	{
...
void dng_filter_warp::ProcessArea (uint32 ,
								   dng_pixel_buffer &srcBuffer,
								   dng_pixel_buffer &dstBuffer)
	{
...
dng_opcode_WarpRectilinear::dng_opcode_WarpRectilinear (dng_stream &stream)
	...
	for (uint32 plane = 0; plane < fWarpParams.fPlanes; plane++)
		...
		fWarpParams.fRadParams.fData [plane][0] = stream.Get_real64 (); 
		fWarpParams.fRadParams.fData [plane][2] = stream.Get_real64 (); 
		fWarpParams.fRadParams.fData [plane][4] = stream.Get_real64 (); 
		fWarpParams.fRadParams.fData [plane][6] = stream.Get_real64 (); 
...
void dng_opcode_WarpRectilinear::PutData (dng_stream &stream) const
	...
	for (uint32 plane = 0; plane < fWarpParams.fPlanes; plane++)
		...
		stream.Put_real64 (fWarpParams.fRadParams.fData [plane][0]); 
		stream.Put_real64 (fWarpParams.fRadParams.fData [plane][2]); 
		stream.Put_real64 (fWarpParams.fRadParams.fData [plane][4]); 
		stream.Put_real64 (fWarpParams.fRadParams.fData [plane][6]); 
...
uint32 dng_opcode_WarpRectilinear::ParamBytes (uint32 planes)
	...
	return (1 * (uint32) sizeof (uint32)		 ) +  
		   (6 * (uint32) sizeof (real64) * planes) +  
		   (2 * (uint32) sizeof (real64)		 );	  
...
uint32 dng_opcode_WarpRectilinear2::ParamBytes (uint32 planes)
	...
	return (1		  * (uint32) sizeof (uint32)		 ) + 
		   (warpTerms * (uint32) sizeof (real64) * planes) + 
		   (2		  * (uint32) sizeof (real64)		 ) + 
		   (1		  * (uint32) sizeof (uint32)		 );	 
...
bool dng_opcode_FixVignetteRadial::IsValidForNegative (const dng_negative & ) const
	{
