...
#define AMAZE_LOG_TAG "PLog_AMaZE"
#define AMAZE_LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, AMAZE_LOG_TAG, __VA_ARGS__)
...
unsigned libraw_fc(LibRaw* libraw, int r, int c)
{
    const unsigned f = libraw->FC(r, c);
    return f == 3 ? 1 : f;
}

unsigned short toUshort(float value)
{
    const float clamped = std::max(0.f, std::min(65535.f, value));
...
void LibRaw::amaze_interpolate()
{
    const int fullWidth = imgdata.sizes.width;
    const int fullHeight = imgdata.sizes.height;

    if (fullWidth < 96 || fullHeight < 96) {
        ahd_interpolate();
        return;
    }

    RUN_CALLBACK(LIBRAW_PROGRESS_INTERPOLATE, 0, 1);

    const int winx = 0;
    const int winy = 0;
    const int winw = fullWidth;
    const int winh = fullHeight;
    const size_t chunkSize = 1;

    array2D<float> rawData(fullWidth, fullHeight, ARRAY2D_CLEAR_DATA);
    array2D<float> red(fullWidth, fullHeight, ARRAY2D_CLEAR_DATA);
    array2D<float> green(fullWidth, fullHeight, ARRAY2D_CLEAR_DATA);
    array2D<float> blue(fullWidth, fullHeight, ARRAY2D_CLEAR_DATA);

    for (int row = 0; row < fullHeight; ++row) {
        for (int col = 0; col < fullWidth; ++col) {
            const unsigned rawChannel = FC(row, col);
            const unsigned rtChannel = rawChannel == 3 ? 1 : rawChannel;
            const float sample = image[row * fullWidth + col][rawChannel];
            rawData[row][col] = sample;
            switch (rtChannel) {
                case 0: red[row][col] = image[row * fullWidth + col][rawChannel]; break;
                case 1: green[row][col] = image[row * fullWidth + col][rawChannel]; break;
                case 2: blue[row][col] = image[row * fullWidth + col][rawChannel]; break;
            }
        }
    }

    const unsigned int cfarray[2][2] = {{libraw_fc(this, 0,0), libraw_fc(this, 0,1)}, {libraw_fc(this, 1,0), libraw_fc(this, 1,1)}};
    const int width = winw, height = winh;
    float initialGain = 1.0f;
    for (int c = 0; c < 4; ++c) {
        const float gain = pre_mul[c];
        if (gain > initialGain) {
            initialGain = gain;
        }
    }
    const float clip_pt = 1.0f / initialGain;
    const float clip_pt8 = 0.8f / initialGain;
    AMAZE_LOGD("amaze_interpolate: initialGain=%f clip_pt=%f clip_pt8=%f pre_mul=[%f,%f,%f,%f]",
               initialGain,
               clip_pt,
               clip_pt8,
               pre_mul[0],
...
#define AMAZETS 160
    ...
    for (int row = 0; row < fullHeight; ++row) {
        for (int col = 0; col < fullWidth; ++col) {
            ushort* pix = image[row * fullWidth + col];
            pix[0] = toUshort(red[row][col]);
            pix[1] = pix[3] = toUshort(green[row][col]);
            pix[2] = toUshort(blue[row][col]);
        }
    }

    border_interpolate(16);
    RUN_CALLBACK(LIBRAW_PROGRESS_INTERPOLATE, 1, 1);
}
