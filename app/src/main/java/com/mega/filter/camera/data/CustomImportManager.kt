...
import com.mega.filter.camera.frame.FrameElement
import com.mega.filter.camera.frame.FrameTemplate
import com.mega.filter.camera.frame.FrameTemplateParser
import com.mega.filter.camera.lut.LutInfo
import com.mega.filter.camera.lut.XmpLutParser
...
class CustomImportManager(private val context: Context) {
    ...
    fun copyFrame(frame: com.mega.filter.camera.frame.FrameInfo, copyName: String): String? {
        return try {
            val frameId = "custom_${UUID.randomUUID()}"
    ...
    fun exportFrameJson(frame: com.mega.filter.camera.frame.FrameInfo): ByteArray? {
        return try {
    ...
    fun getCustomFrames(): List<com.mega.filter.camera.frame.FrameInfo> {
        return try {
            ...
            val frameList = mutableListOf<com.mega.filter.camera.frame.FrameInfo>()
            for (i in 0 until jsonArray.length()) {
                ...
                frameList.add(
                    com.mega.filter.camera.frame.FrameInfo(
                        id = id,
                        path = path,
                        nameMap = nameMap,
                        previewResId = 0,
