...
import com.mega.filter.camera.lut.ChromaDenoiseAlgorithm
import com.mega.filter.camera.ml.SharedDepthEstimator
...
import com.mega.filter.camera.processor.GlesMgcRawSpatialStacker
import com.mega.filter.camera.processor.GlesPixelBufferTransfer
...
import com.mega.filter.camera.processor.GpuStackCompletionTimeline
import com.mega.filter.camera.processor.MgcSpatialOutputMode
import com.mega.filter.camera.processor.MgcMergeMethod
import com.mega.filter.camera.processor.MgcRawProcessorPipeline
import com.mega.filter.camera.processor.RawNoiseModel
...
import com.mega.filter.camera.processor.RawStackResult
import com.mega.filter.camera.utils.BitmapUtils
import com.mega.filter.camera.utils.DirectBufferPixelPacker
...
class RawDemosaicProcessor {
    ...
    private fun obtainReadbackBuffer(pixelSize: Int): ByteBuffer {
        ...
        releaseReadbackBuffer()
        return (com.mega.filter.camera.utils.DirectBufferAllocator.allocateNative(pixelSize.toLong())
            ?.order(ByteOrder.nativeOrder())
    ...
    private fun releaseReadbackBuffer() {
        readbackBuffer?.let { com.mega.filter.camera.utils.DirectBufferAllocator.freeNative(it) }
        readbackBuffer = null
