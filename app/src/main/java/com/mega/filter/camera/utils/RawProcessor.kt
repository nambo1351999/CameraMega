...
import com.mega.filter.camera.raw.DngBaselineExposure
...
import com.mega.filter.camera.raw.RawNoiseProfileLayout
import com.mega.filter.camera.raw.RawDemosaicProcessor
import com.mega.filter.camera.raw.RawRenderingEngine
import com.mega.filter.camera.raw.RawSceneExposureDeviceLimits
import com.mega.filter.camera.raw.RawWhiteLevelCorrection
import com.mega.filter.camera.raw.toAdobeDefaultMeteringPlan
import java.io.File
...
object RawProcessor {
    ...
    suspend fun prepareRawDngProfile(
        ...
        val captureProfile = options.captureProfilePreparer?.prepare(
            com.mega.filter.camera.raw.RawDngCaptureProfileInput(
        ...
        val exposureOffsetEv = captureProfile?.exposureOffsetEv
            ?.takeIf { it.isFinite() }
            ?.coerceIn(
                com.mega.filter.camera.raw.MeteringSystem.RAW_EXPOSURE_MIN_EV,
                com.mega.filter.camera.raw.MeteringSystem.RAW_EXPOSURE_MAX_EV,
            )
