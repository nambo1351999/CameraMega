...
import com.zoomx.mega.cameramega.lut.XmpLutParser
...
class CustomImportManager(private val context: Context) {
    ...
    fun copyFrame(frame: com.zoomx.mega.cameramega.frame.FrameInfo, copyName: String): String? {
        return try {
            val frameId = "custom_${UUID.randomUUID()}"
    ...
    fun exportFrameJson(frame: com.zoomx.mega.cameramega.frame.FrameInfo): ByteArray? {
        return try {
    ...
    fun getCustomFrames(): List<com.zoomx.mega.cameramega.frame.FrameInfo> {
        return try {
            ...
            val frameList = mutableListOf<com.zoomx.mega.cameramega.frame.FrameInfo>()
            for (i in 0 until jsonArray.length()) {
                ...
                frameList.add(
                    com.zoomx.mega.cameramega.frame.FrameInfo(
                        id = id,
                        path = path,
                        nameMap = nameMap,
                        previewResId = 0,
